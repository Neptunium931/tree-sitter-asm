fn main() {
    ...
    if !"queries/asm/highlights.scm".is_empty() && std::path::Path::new("queries/asm/highlights.scm").exists() {
        println!("cargo:rustc-cfg=with_highlights_query");
    }
    println!("cargo:rustc-check-cfg=cfg(with_injections_query)");
    if !"queries/asm/injections.scm".is_empty() && std::path::Path::new("queries/asm/injections.scm").exists() {
